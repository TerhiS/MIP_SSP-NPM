...
for (ins in 1:length(data.list)) {
  ...
  for (obj in c("flowtime", "makespan")) {
    ...
    # heuristic identifier "CH" for results of construction heuristic
    heur <- "CH"
    ...
    #### retrieve initial solution and objective values from construction heuristic result file ("CH")
    #### Note: You can skip this step if you do not want to separate the construction and ILS file!
    setwd("example_results/")
    solution <-
      read.csv(
    ...
    while (pert_again == T) {
      ...
      if (pert_count %in% time_pert) {
        ...
        bs_pert[which(time_pert == pert_count), 1] <-
          switch_opt
        bs_pert[which(time_pert == pert_count), 2] <-
          tft_opt
        bs_pert[which(time_pert == pert_count), 3] <-
          fmax_opt
        bs_pert[which(time_pert == pert_count), 4] <-
          paste0(timep[[1]])
        tic.clearlog()
      }
      
      # if solution not improved, but 100 perturbation moves not yet performed
    ...
    loads_opt <-
      capture.output(cat(paste0(strwrap(loads_opt))))
    
    # Summary of best known results to file
    setwd("example_results/")
    f_bs <- paste0("./bks_", obj, ins, ".csv", sep = ";")
    write.table(
      bs_pert,
      paste("./bks_", obj, ins, ".csv", sep = ""),
      sep = ";",
      row.names = T,
      col.names = F
    )
    ...
    mat <-
      as.matrix(rbind(switch_opt,
                      ...
                      loads_opt))
    f_ils <-
      paste0("./ILS_", pert_iter, "_", obj, ins, ".csv", sep = "")
    # write output to file
    if (file.exists(f_ils) == T) {
      print("file already exists")
    }
    if (file.exists(f_ils) == F) {
      write.table(
        mat,
        paste("./ILS_", pert_iter, "_", obj, ins, ".csv", sep = ""),
        sep = ",",
        row.names = T,
        col.names = F
      )
