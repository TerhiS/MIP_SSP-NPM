...
for (instance in 1:length(data.list)) {
  ...
  while (sum(j_left) > 0) {
    ...
    if (f[free_m] == 0) {
      ...
      pi_m_SPT[(length(pi_m_SPT[, free_m][!is.na(pi_m_SPT[, free_m])]) +
                  1), free_m] <- j_min
    ...
    # if the job is not the first job then tool swtiches have to be considered
      ...
      # of all jobs left
      previous_j <-
        pi_m_SPT[(length(pi_m_SPT[, free_m][!is.na(pi_m_SPT[, free_m])])), free_m]
  ...
  for (m in 1:max_m) {
    if (length(l_SPT[[m]]) == 1) {
      ...
      t_req_m[[m]] <-
        unlist(union(NULL, as.vector(unlist(req_t[l_SPT[[m]][-1]]))))
  ...
  f_ch <- paste0("./cH", instance, ".csv", sep = "")
